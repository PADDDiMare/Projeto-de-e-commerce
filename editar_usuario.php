	...
	if(!isset($_GET["id"])) { // Inclusão
		...
		if(isset($_POST["btnIncluir"])) {
			...
			if($senha == $confirmacaoSenha) {
				...
				if(!is_null($id)) {
					header("location:listar_usuario.php?id=$id&msg=incluir-sucesso");
				} else { 
		...
		<div class="col-md-12">
			<form name="form" id="form" method="post" action="#">
				...
				<div class="form-group">
					...
					<input type="submit" name="btnExcluir" id="btnExcluir" class="btn btn-danger" value="Excluir" onclick="return confirm('Deseja  excluir esse registro?');" />
					<?php } ?>
				</div>
			</form>
		</div>
